...
[快速开始](#快速开始) • [系统架构](#系统架构) • [功能特性](#功能特性) • [文档](#文档) • [比较模式](#-比较评测模式新)
...
| 模式 | 说明 | 适用场景 |
|------|------|---------|
| **打分模式**（原系统） | 4维度打分（1-5分） | 详细量化评估，获取多维度分数 |
| **比较模式**（新增）| 两两比较选择 | 避免主观偏见，直接判断优劣 |

两个模式**完全独立运行**，使用不同的数据库和端口，互不干扰。详见 [比较评测模式](#-比较评测模式新)。
...
- [比较模式完整文档](README_COMPARE.md) - 详细说明
- [快速启动指南](docs/快速启动_比较模式.txt) - 中文
- [数据库结构](db/schema_compare.sql)
...
传统的打分评测容易受到主观因素影响，比较评测模式采用**两两比较**的方式，让评审员直接判断哪个生成视频更好，避免评分尺度不一致的问题。

### 主要特性

- ✅ **两两配对**：每个参考视频的N个生成视频，生成C(N,2)个比较任务
- ✅ **简化评测**：只需选择更好的一个，无需打分
- ✅ **独立运行**：使用独立的数据库、端口、视频目录
- ✅ **动态监控**：支持运行时添加新模型和视频
- ✅ **三人评测制**：每个比较任务需3个评审员
...
| **UI布局** | 上下排列 | 左参考，右两个生成上下叠放 |
| **任务数** | 每个生成视频1个 | 每对生成视频1个 |
| **启动命令** | lan_start_with_monitor.ps1 | lan_start_compare.ps1 |
| **停止命令** | lan_stop.ps1 | lan_stop_compare.ps1 |
| **进度查看** | check_progress.py | check_progress_compare.py |
| **导出数据** | export_all.ps1 | export_all_compare.ps1 |

### 详细文档

完整的比较模式文档请参考：[README_COMPARE.md](README_COMPARE.md)
